...
%- Basic notions
%- Ex-post compromises
%- 3 individuals: Several SCRs fail to pick an ex-post compromise
%- 2 individuals: failure to pick an ex-post compromise
...
%Appr_quant = n is FB. Appr_quant = q is BK. Appr_quant = n/2 is MC = Bucklin.
...
\begin{frame}[fragile]
	...
	Given $\alts \subseteq \allalts$ and $s: \alts → \R$,
	\begin{align}
		\argmin_{x \in \alts}s(x) &= \set{x \in \alts \suchthat s(x) ≤ s(y), \forall y \in \alts}\\
		&= \argmin_\alts s
	\end{align}
...
\begin{frame}[fragile]
	...
	\ac{ECC} rules are \emph{very} pro-equality
	\begin{example}
		\centering
		\begin{tikzpicture}
			\path node (P) {$\prof$};
...
\begin{frame}[fragile]
	...
	\begin{example}[A PCC and Paretian \ac{SCR}]
		Consider $f^\sigma = \argmin_{\PO(\prof)} (\sigma \circ \lambda_{\prof})$.
		\begin{center}
			\begin{tikzpicture}
				\path node (P) {$\prof$};
				\path (P.south) node[anchor=north] (profile) {$%
			...
			\end{tikzpicture}
		\end{center}
		\onslide<2>{$f^\sigma$ may pick $a$ or $d$, depending on $\sigma$}
	\end{example}
\end{frame}
...
\begin{frame}
	...
	\end{theorem}
	\begin{proof}
		\only<1>{?}\onslide<2>{$\BK[n]$ is Paretian.}
	\end{proof}
\end{frame}
...
\begin{frame}
	...
	\end{proof}
	For example: 
%	\begin{example}[Multiple constant loss vectors]
		$\begin{array}{l}
